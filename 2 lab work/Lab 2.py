...
print(high_salary)
print("Самый высокооплачиваемый сотрудник: ", max_zhumyshy)
file.close()
result2 = open("high_salary.csv", "w", encoding="utf-8")
for emp in high_salary:
    result2.write(emp["name"] + "," + emp["department"] + str(emp["salary"]))
result2.close()

#3
import json
f3 = open("orders.json", "r")
x = json.load(f3)
total_buy = 0
users_spending = {}
total_items_sold = 0
...
for order in x:
    ...
    total_buy += total
    if user_name not in users_spending:
        users_spending[user_name] = 1
    else:
        users_spending[user_name] += 1
    for item in items:
...
f3.close()
result3 = open("summary.json", "w")
summary = {
    "total_revenue": total_buy,
    "top_user": top_user,
    "most_popular_item": most_popular_item,
    "total_orders": total_items_sold
}
json.dump(summary, result3)
print(total_buy)
